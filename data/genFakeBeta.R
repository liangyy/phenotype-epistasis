...
help_text = 'This script generates a fake GWAS summary statistic data sheet in GZ format.
...
Note that a reference SNP list (consider the format SNP list output by genFakeGeno.R) \
is required to match the SNP ID in downstream analysis'

option_list = list(
  make_option(c('-s', '--sigma'), type="numeric", default=0.5,
              help="variance of prior [default = %default]", metavar="character"),
  make_option(c("-f", "--fraction"), type="numeric", default=0.1,
              help="fraction of causal variants [default = %default]", metavar="character"),
  make_option(c("-h", "--heritability"), type="numeric", default=0.1,
              help="heritability of the trait of the GWAS [default = %default]", metavar="character"),
  make_option(c('-n', '--gwas_sample_size'), type='numeric', default=20000,
              help='the sample size of the GWAS to simulate [default = %default]', metavar='character'),
  ...
  make_option(c('-c', '--nchr'), type="numeric", default=22,
              help="the number of chromosomes in reference SNP list [default = %default]", metavar="character"),
  make_option(c('-i', '--drop_rate'), type='numeric', default=0.2,
              help='the fraction of SNPs dropped in output and the same number of new SNPs will be included [default = %default]', metavar='character'),
...
snpid.new <- paste0('snp.new', 1 : sum(selected.snp.indicator == 0))
chr.new <- sample(1 : nchr, 1)
pos.new <- round(runif(opt$nsnp) * 1e6)
A.new <- 'G'
B.new <- 'T'
...
snp.out.list <- snp.out.list[order(snp.out.list$chr, snp.out.list), ]
...
write.table(snp.out.list, gz1, quote = F, col.names = F, row.names = F)
close(gz1)
