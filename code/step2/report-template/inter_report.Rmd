...
for(i in 1 : length(xtyi.files)) {
  ...
  p <- ggplot(df) + geom_point(aes(x = SNP, y = Mean)) + geom_errorbar(aes(x = SNP, ymin = Mean - 1.96 * Std, ymax = Mean + 1.96 * Std), colour = "black", width = .1) + facet_grid(. ~ Type) + coord_flip() + theme(axis.text.x=element_text(angle=90,hjust=1))
